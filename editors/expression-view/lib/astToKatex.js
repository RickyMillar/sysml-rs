...
function paren(inner, childPrec, parentPrec) {
...
export function astToKatex(node) {
...
function renderNode(node) {
    switch (node.kind) {
        ...
        case 'LiteralRational': {
            ...
            if (v !== null)
                return { tex: String(v), prec: PREC.atom };
        ...
        case 'FeatureChainExpression': {
            ...
            if (parts.length === 0 && node.name)
                return { tex: renderName(node.name), prec: PREC.atom };
...
function renderOperator(node) {
    ...
    if (kids.length === 2) {
        ...
        const rhsAllowed = op === '**' || op === '^' ? prec : (prec + 1);
...
function renderInvocation(node) {
    ...
    switch (name) {
        case 'abs':
            if (args.length === 1)
                return { tex: `\\left|${args[0]}\\right|`, prec: PREC.atom };
            break;
        case 'sqrt':
            if (args.length === 1)
                return { tex: `\\sqrt{${args[0]}}`, prec: PREC.atom };
            break;
        case 'sin':
        case 'cos':
        case 'tan':
        ...
        case 'exp':
            if (args.length === 1)
                return { tex: `\\${name}\\left(${args[0]}\\right)`, prec: PREC.postfix };
            break;
    }
    return {
...
function renderCollection(node, label) {
    if (node.children.length >= 2) {
        const src = renderNode(node.children[0]);
...
//# sourceMappingURL=astToKatex.js.map