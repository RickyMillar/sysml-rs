...
/** The 8 standard view supertypes (book ch13; mirrors sysml-views.json). */
export interface ViewTypeInfo {
  /** Supertype token as it appears in source (`view def X :> <token>`). */
  token: string;
  label: string;
...
/**
 * The renderer's REAL 8 kinds (`bare_name_to_view_type`,
 * view_request.rs) — NOT the book's list: `Requirement`/`UseCase`
 * supertypes are legal source but resolve to General via the stdlib
 * walk (see requirements-parametric-retirement.md), so offering them
 * as cards would promise projections that don't exist.
 */
export const VIEW_TYPES: readonly ViewTypeInfo[] = [
  { token: 'General', label: 'General', blurb: 'Any elements as a plain node graph — the safe default.' },
  { token: 'Interconnection', label: 'Interconnection (IBD)', blurb: 'Parts with their ports and the connections between them.' },
  { token: 'StateTransition', label: 'State transition', blurb: 'States and the transitions between them.' },
  { token: 'ActionFlow', label: 'Action flow', blurb: 'Actions and the control/object flow between them.' },
  { token: 'Sequence', label: 'Sequence', blurb: 'Lifelines and the messages exchanged over time.' },
  { token: 'Browser', label: 'Browser', blurb: 'A containment tree of the exposed elements.' },
  { token: 'Grid', label: 'Grid', blurb: 'A tabular projection of the exposed elements and their attributes.' },
  { token: 'Geometry', label: 'Geometry', blurb: 'Spatial layout of parts (geometry-bearing models).' },
] as const;
 ...
 * Header shapes handled: `view scratch : Interconnection {` (usage,
 * post-F14) and `view def scratch :> Interconnection {` (def form).
