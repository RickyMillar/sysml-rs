...
mod tests {
    ...
    fn make_crossref_multi(property: &str, target: &str, rule: &str) -> CrossReference {
        CrossReference {
            property: property.to_string(),
            target_type: target.to_string(),
            namespace: "SysML".to_owned(),
    ...
    #[test]
    fn crossrefs_between_different_types() {
        let refs = vec![
            make_crossref("general", "Type", "Specialization"),
            make_crossref("importedElement", "Element", "MembershipImport"),
        ];

        let code = generate_crossref_registry(&refs);

        // Both target types should appear as group comments
        assert!(code.contains("// Target: Type"));
        assert!(code.contains("// Target: Element"));
        assert!(code.contains("\"general\""));
        assert!(code.contains("\"importedElement\""));
    }

    #[test]
    fn multi_valued_crossref_generates_is_multi_true() {
        let refs = vec![make_crossref_multi("supplier", "Element", "Dependency")];

        let code = generate_crossref_registry(&refs);

        assert!(
            code.contains("is_multi: true"),
            "multi-valued crossref should have is_multi: true"
        );
    }

    #[test]
    fn single_valued_crossref_generates_is_multi_false() {
        let refs = vec![make_crossref("general", "Type", "Specialization")];

        let code = generate_crossref_registry(&refs);

        assert!(
            code.contains("is_multi: false"),
            "single-valued crossref should have is_multi: false"
        );
    }

    #[test]
    fn empty_crossrefs_generates_empty_array() {
        let refs: Vec<CrossReference> = vec![];

        let code = generate_crossref_registry(&refs);

        assert!(code.contains("pub const ALL_CROSS_REFERENCES: &[CrossRefSpec] = &[\n];"));
    }

    #[test]
    fn generated_code_contains_helper_functions() {
        let refs = vec![make_crossref("general", "Type", "Specialization")];

        let code = generate_crossref_registry(&refs);

        assert!(code.contains("pub fn find_crossref_spec("));
        assert!(code.contains("pub fn crossrefs_for_target("));
        assert!(code.contains("pub fn crossrefs_for_strategy("));
        assert!(code.contains("pub fn is_crossref_property("));
        assert!(code.contains("pub fn crossref_stats()"));
    }
}
