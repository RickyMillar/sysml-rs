...
use crate::{AssignmentOp, ParallelStepResult, StateMachineIR, TransitionActionIR};
...
impl ParallelStateMachineRunner {
    ...
    pub fn from_graph(graph: &ModelGraph) -> Result<Self, Vec<Diagnostic>> {
        use super::StateMachineCompiler;
        use crate::CompileToIR;
    ...
    pub fn send(&mut self, event: &str) -> ParallelStepResult {
        self.event_queue.push_back(event.to_owned());
        ...
        match action {
            ...
            TransitionActionIR::Structured {
                ...
                for assign in assignments {
                    ...
                    let evaluated = assign.value_expr.as_ref().and_then(|expr| {
                        let mut eval_ctx = EvalContext::new();
                        for (name, value) in context.iter() {
                            eval_ctx.set(name.clone(), Value::Float(*value));
                        }
                        evaluator.eval(expr, &eval_ctx).ok()
                    });
                    ...
                    let new_value = match assign.operator {
                        AssignmentOp::Set => operand,
                        AssignmentOp::Add => current + operand,
                        AssignmentOp::Subtract => current - operand,
                    };
...
mod tests {
    use super::*;
    use crate::{AssignmentIR, RegionIR, StateIR, TransitionIR};
    ...
    fn internal_events_cross_region() {
        ...
        let ir = StateMachineIR::parallel("CrossRegion")
            .with_region(
                RegionIR::new("regionA", "A1")
                    .with_state(StateIR::new("A1"))
                    .with_state(StateIR::new("A2").with_entry_action(
                        TransitionActionIR::structured(vec![], vec!["triggerB".to_string()]),
                    ))
    ...
    fn timing_context_updates() {
        let ir = StateMachineIR::parallel("TimingTest").with_region(
            RegionIR::new("timer", "waiting")
                ...
                .with_state(StateIR::new("processing").with_entry_action(
                    TransitionActionIR::structured(vec![AssignmentIR::add("t_ms", 10.0)], vec![]),
                ))
    ...
    fn execute_until_stable() {
        ...
        let ir = StateMachineIR::parallel("ChainTest")
            ...
            .with_region(
                RegionIR::new("regionB", "B1")
                    .with_state(StateIR::new("B1"))
                    .with_state(StateIR::new("B2").with_entry_action(
                        TransitionActionIR::structured(vec![], vec!["triggerC".to_string()]),
                    ))
                    .with_transition(TransitionIR::new("B1", "B2").with_event("triggerB")),
            )
            .with_region(
