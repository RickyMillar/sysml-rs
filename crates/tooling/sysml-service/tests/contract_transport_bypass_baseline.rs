...
// ---------------------------------------------------------------------------
// Bucket B / B1 — diagram peers (open / view / expand)
// LSP handlers: commands.rs L2329 handle_diagram_open, L2440 handle_diagram_view,
//               L2584 handle_diagram_expand
// Service commands (planned, P1):
//   sysml.diagram.open(uri, view_type) -> SModelJson
//   sysml.diagram.view(uri, view_type) -> SModelJson
//   sysml.diagram.expand(uri, view_type, expanded_node_ids: Vec<String>) -> SModelJson
// ---------------------------------------------------------------------------
//
// All 3 LSP handlers open-code the SModel projection (`build_set_model_params`
// → `generate_diagram` → `sysml_diagram::smodel::to_smodel*`) AND mutate
// service-owned `diagram_manager()` state (`open_diagrams`, `expanded_nodes`)
// from the transport. Per design §B1: the SModel projection primitive
// (`build_set_model_params`) and its diagnostic-overlay step move INTO
// `sysml-service`; the LSP becomes a thin marshal over the SModel return.
//
// Return shape (all 3): a `serde_json::Value` carrying the sprotty-compatible
// SModel JSON tree. Top-level shape: `{ id, type, children: [...] }` with
// `id: "graph"`, `type: "graph"` for the root SGraph. SModel-level CSS
// classes (e.g. "diagnostic-error") are present on nodes whose source_range
// overlaps with a publish-tier diagnostic — that overlay step also moves
// onto the service.
//
// These baselines pin the SModel root shape so P1 lands additively. They
// stay ignored until the service commands exist.

/// Baseline regression gate for the planned `sysml.diagram.open` service command.
///
/// Pins SModel root shape `{ id: "graph", type: "graph", children: [...] }`.
#[test]
fn baseline_diagram_open_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_diagram_open.sysml";
    let source = r#"
        package DiagP {
            part def Widget {
                attribute mass : Real = 1.0;
    ...
    .expect("sysml.diagram.open must succeed");

    let obj = result
        .as_object()
        .expect("diagram.open result must be a JSON object (SGraph)");

    for k in ["id", "type", "children"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.diagram.open` lost SModel root field `{k}`. \
             LSP handler at commands.rs L2329 emits an SGraph; P1 must preserve it."
        );
    }
    assert_eq!(
        obj.get("type").and_then(|v| v.as_str()),
        Some("graph"),
        "SModel root.type must be \"graph\""
    );
    assert!(
        obj.get("children").and_then(|v| v.as_array()).is_some(),
        "SModel root.children must be a JSON array"
    );
...
/// Baseline regression gate for the planned `sysml.diagram.view` service command.
#[test]
fn baseline_diagram_view_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_diagram_view.sysml";
    let source = r#"
        package DiagP {
            part def Widget {
                attribute mass : Real = 1.0;
    ...
    .expect("sysml.diagram.view must succeed");

    let obj = result
        .as_object()
        .expect("diagram.view result must be a JSON object (SGraph)");

    for k in ["id", "type", "children"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.diagram.view` lost SModel root field `{k}`."
        );
    }
    assert_eq!(
        obj.get("type").and_then(|v| v.as_str()),
...
/// Baseline regression gate for the planned `sysml.diagram.expand` service command.
#[test]
fn baseline_diagram_expand_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_diagram_expand.sysml";
    let source = r#"
        package DiagP {
            part def Widget {
                attribute mass : Real = 1.0;
    ...
    // Expand with an empty expanded-node set — round-trips the projection
    // without requiring a particular state-machine fixture.
    let result = execute_command(
        &service,
        ...
        json!({
            "uri": uri,
            "view_type": "general",
            "expanded_node_ids": serde_json::Value::Array(vec![]),
        }),
    )
    .expect("sysml.diagram.expand must succeed");

    let obj = result
        .as_object()
        .expect("diagram.expand result must be a JSON object (SGraph)");

    for k in ["id", "type", "children"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.diagram.expand` lost SModel root field `{k}`."
        );
    }
    assert_eq!(
        obj.get("type").and_then(|v| v.as_str()),
...
// ---------------------------------------------------------------------------
// Bucket B / B5 — export.smodel
// CLI handler: sysml-cli/src/export.rs L114 `run_smodel`
// Service command (planned, P1):
//   sysml.export.smodel(uri, view, expand_all) -> SModelJson
// ---------------------------------------------------------------------------
//
// Per design §B5: the CLI `run_smodel` open-codes load_file + require_graph +
// elaborate + `sysml_diagram::smodel::to_smodel` + `expand_all` driven HashSet.
// P1 collapses the CLI to a single `service.export_smodel(uri, view, expand_all)`
// call and prints the returned JSON. The 10-view enum re-exports from
// `sysml-service` so the service command takes it as a typed arg (no string
// round-trip).
//
// Return shape: a `serde_json::Value` carrying the SModel JSON tree
// (sprotty-compatible). Top-level: `{ id, type: "graph", children: [...] }`.

/// Baseline regression gate for the planned `sysml.export.smodel` service command.
#[test]
fn baseline_export_smodel_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_export_smodel.sysml";
    let source = r#"
        package ExpP {
            part def Widget {
                attribute mass : Real = 1.0;
    ...
    .expect("sysml.export.smodel must succeed");

    let obj = result
        .as_object()
        .expect("export.smodel result must be a JSON object (SGraph)");

    for k in ["id", "type", "children"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.export.smodel` lost SModel root field `{k}`. \
             CLI handler at export.rs L114 emits an SGraph; P1 must preserve it."
        );
    }
    assert_eq!(
        obj.get("type").and_then(|v| v.as_str()),
...
// ---------------------------------------------------------------------------
// Bucket B / B2 — visualization peers (flow.visualize + action.visualize)
// LSP handlers:
//   - handle_flow_visualize   (sysml-lsp-server/src/commands.rs L1801)
//   - handle_action_visualize (sysml-lsp-server/src/commands.rs L1753)
// Service commands (planned, B2 P1):
//   sysml.flow.visualize(uri, flow_id: Option<&str>)
//       -> { plantuml: String, smodel: serde_json::Value }
//   sysml.action.visualize(uri, action_id: &str)
//       -> { plantuml: String, smodel: serde_json::Value }
// ---------------------------------------------------------------------------
//
// commands return BOTH PlantUML text AND the SModel JSON via a single graph
// walk — transports pick whichever they need. The "dual-shape" convention is
// established here on its first arc; subsequent rendering commands will peer
// it. The current LSP `handle_flow_visualize` already returns plantuml+smodel
// but under different keys (`format`/`content`/`smodel`); P1 lifts both to
// the design-target shape `{plantuml, smodel}`. `handle_action_visualize`
// today returns only a status object; P1 fills in plantuml from a new
// `sysml_diagram::to_plantuml_activity` primitive driven off ActionGraphIR.

/// Baseline regression gate for the planned `sysml.flow.visualize` service command.
///
/// Pins dual-shape return convention: top-level keys MUST be exactly `plantuml`
/// (String) and `smodel` (object — SModel root shape `{id, type:"graph", children}`).
#[test]
fn baseline_flow_visualize_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_flow_visualize.sysml";
    // Minimal model with two parts and a flow connection between them.
    let source = r#"
        package FlowP {
            part def Source {
                out port outPort;
            ...
            part src : Source;
            part snk : Sink;
            flow src.outPort to snk.inPort;
        }
    "#;
    service
        .load_source(uri, source)
    ...
    .expect("sysml.flow.visualize must succeed");

    let obj = result
        .as_object()
        .expect("flow.visualize result must be a JSON object {plantuml, smodel}");

    for k in ["plantuml", "smodel"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.flow.visualize` lost top-level field `{k}`. \
             Design §B2 mandates dual-shape `{{plantuml, smodel}}` return."
        );
    }
    ...
    let plantuml = obj
        .get("plantuml")
        .and_then(|v| v.as_str())
        .expect("plantuml must be a String");
    assert!(
        plantuml.starts_with("@startuml") && plantuml.contains("@enduml"),
        "plantuml field must contain a PlantUML document (@startuml…@enduml)"
    );

    let smodel = obj
        .get("smodel")
        .and_then(|v| v.as_object())
        .expect("smodel must be a JSON object (SGraph)");
    for k in ["id", "type", "children"] {
        assert!(
            smodel.contains_key(k),
            "SModel root lost field `{k}` in flow.visualize"
        );
    }
    assert_eq!(
        smodel.get("type").and_then(|v| v.as_str()),
        Some("graph"),
        "smodel root.type must be \"graph\""
    );

    // Sanity reference — keep ElementKind import live.
    let _ = ElementKind::PartUsage;
}

/// Baseline regression gate for the planned `sysml.action.visualize` service command.
///
/// Pins dual-shape return convention identical to flow.visualize, but driven by
/// a single named action.
#[test]
fn baseline_action_visualize_shape() {
    let service = SysmlService::empty();

    let uri = "inline://baseline_action_visualize.sysml";
    // Minimal model with a single action definition the visualizer can compile.
    let source = r#"
        package ActP {
            action def DoIt {
                first start;
    ...
    .expect("sysml.action.visualize must succeed");

    let obj = result
        .as_object()
        .expect("action.visualize result must be a JSON object {plantuml, smodel}");

    for k in ["plantuml", "smodel"] {
        assert!(
            obj.contains_key(k),
            "P1 regression: service `sysml.action.visualize` lost top-level field `{k}`. \
             Design §B2 mandates dual-shape `{{plantuml, smodel}}` return."
        );
    }
    ...
    for k in ["id", "type", "children"] {
        assert!(
            smodel.contains_key(k),
            "SModel root lost field `{k}` in action.visualize"
        );
    }
    assert_eq!(
        smodel.get("type").and_then(|v| v.as_str()),
